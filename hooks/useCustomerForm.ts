...
export const useCustomerForm = (initialCart: ICartItem[], initialMethod: PaymentMethod = 'COD') => {
  ...
  const isFormValid = (cartItems: ICartItem[]) => {
    ...
    if (!isPhoneValid(phone.trim())) return (toast.error('Invalid phone number'), false);
    if (email && !isEmailValid(email.trim())) return (toast.error('Invalid email'), false);
    if (!shippingAddress.trim()) return (toast.error('Shipping address is required'), false);
    if (paymentMethod === 'ONLINE' && !paymentImage) return (toast.error('Payment slip is required'), false);
